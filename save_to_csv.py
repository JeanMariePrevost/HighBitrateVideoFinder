...
def save_to_csv(video_files):
    ...
    output_file = os.path.join(
        results_dir,
        f"video_files_{datetime.datetime.now().strftime('%Y%m%d%H%M%S')}.csv",
    )
    ...
    with open(output_file, mode="w", newline="", encoding="utf-8-sig") as file:
        ...
        for video_file in video_files:
            # print(f"Processing {video_file.file_path}...")
            file = video_file.file_path
            bitrate_in_mbps = try_extract(
                lambda: f"{video_file.get_bitrate() / 1_000_000:.1f}", file, "bitrate"
            )
            size_in_mb = try_extract(
                lambda: f"{video_file.get_filesize() / (1024 * 1024):.1f}",
                file,
                "filesize",
            )
            length = try_extract(
                lambda: f"{video_file.get_duration():.1f}", file, "duration"
            )
